...
- Todo tasks now remain pending until explicitly started, open work survives compaction, and false completion reminders stop at the configured limit.
...
- Todo state now stays consistent across direct and eval tool calls, slash commands, RPC, Cursor, session reloads, and collaboration guests.
...
- Arrow-key prompt recall now offers the active session's prompts followed by the rest of the project's, so a session that persisted few prompts keeps its project history.
...
- OpenAI usage in the status line no longer displays account plan names such as `plus`, `pro`, or `prolite`.

### Fixed

- Fixed `todo` and other tools called through eval rejecting optional `None`/`null` arguments that direct tool calls accept.
- Arrow-key prompt recall now switches to the active session after starting, resuming, branching, or deleting a session.
